...
//   
  ...
  int matchedCount = 0; //matchedCount indicates that we have matched all 4 muons appropriately, 2 to the Z, 2 to an UpsiN, and we have determined what that N is
   ...
   //trigger counters
int  firesDoubleMuOnly2016 = 0;
int firesDoubleMuOnly2017 = 0;
int firesDoubleMuOnly2018 = 0;
 ...
 // double mu_mu_from_Z_Prob_Cut = 0.05; 
  ...
  TFile *ntuple = new TFile("firesDoubleMuonOnly_2017Data_tightMassCut.root", "RECREATE");
  TTree *aux;
...
for (int jj=0; jj<(int)temp_big4MuVtxProb.size(); jj++)   {
  ...
  if (my_counter == (double)temp_big4MuVtxProb.size()){
    if (Z_mass > 100. || Z_mass < 80.){
      continue;
    }
    if (upsi_mass > 9.6 || upsi_mass < 9.3){
      continue;
    }
    aux->Fill();
  }
...
std::cout << "firesDoubleMuOnly2016:  " << firesDoubleMuOnly2016 << std::endl;
std::cout << "firesDoubleMuOnly2017:  " << firesDoubleMuOnly2017 << std::endl;
std::cout << "firesDoubleMuOnly2018:  " << firesDoubleMuOnly2018 << std::endl;
