...
//   
 ...
 //Cut so that we only look to the left of the Y(1S) peak. Go all the way down to 8 to try to get more events.
 
  //From MC, we know the the sigma of the Y(1S) Gaussian is, to close enough approximation for our purposes, 0.1 GeV.
  
  double upsi1S_sig_mass_window = 0.1;
...
//  double upsi_mass_low_phase2 = upsi1S_true_mass - (num_of_sig_mass_windows_low_end * upsi1S_sig_mass_window); //8.5;
 ...
 // double mu_mu_from_Z_Prob_Cut = 0.05; 
  ...
  TFile *ntuple = new TFile("ntuple_2018Data_cut_and_count_left_expanded.root", "RECREATE");
  TTree *aux;
